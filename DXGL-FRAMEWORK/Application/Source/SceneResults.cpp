...
void SceneResults::Update(double dt) {
	BaseScene::Update(dt);
	ClearDebugText();
...
void SceneResults::Exit() {
	BaseScene::Exit();


}

void SceneResults::HandleKeyPress() {
...
void SceneResults::RenderObj(const std::shared_ptr<RObj> obj) {
...
void SceneResults::RenderMesh(GEOMETRY_TYPE type, bool enableLight) {
...
void SceneResults::InitDebugText(GEOMETRY_TYPE font) {
	auto& newObj = RObj::newObject;
	for (int i = 0; i < 10; i++) {
...
bool SceneResults::AddDebugText(const std::string& text, int index) {
...
void SceneResults::ClearDebugText() {
	for (auto& obj_weak : debugTextList)
